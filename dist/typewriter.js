                ...
                this.print = _.debounce(function (text) {
                    ...
                    _this.$timeout(function () {
                        if (letter != null) {
                            ...
                            _this.$scope.$emit('TypewriterLine:done');
                        }
                    }, delay);
                }, this.delay / 2);
                this.print(text);
...
(function (typewriter) {
    var WriterCtrl = (function () {
        function WriterCtrl($timeout, $scope, $attrs) {
            ...
            this.print = function () {
                ...
                if (_this.tempLines)
                    _this.$timeout(function () {
                        _this.result.push(_this.tempLines.shift());
                    });
            };
            $scope.$watch($attrs.lines, function () { return _this.print(); });
            $scope.$watch($attrs.printFn, function () {
                _this.exposePrint();
            });
    ...
    var WriterDirective = (function () {
        function WriterDirective() {
            ...
            this.template = "<tt-line ng-repeat='line in W.result track by $index' text='line' delay='{{W.delay}}'>";
        }
